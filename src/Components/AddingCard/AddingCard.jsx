...
function AddingCard(props) {
  ...
  function submitMod() {
    if (
      newfilm.name.length === 0 ||
      newfilm.description.length === 0 ||
      newfilm.rating.length === 0 ||
      newfilm.posterURL.length === 0 ||
      newfilm.trailer.length === 0
    ) {
      alert("Erreur : fill all inputs please");
    } else if (!newfilm.trailer.includes("https://www.youtube.com/")) {
      alert("Erreur : Trailer URL is Invalid");
    } else {
      props.show();
      props.onadd(newfilm);
      setNewfilm({
      ...
      {!props.addingOptions && (
        ...
        <span className="astar" onMouseEnter={() => b+5} style={b>4 ? {opacity : "0.6"} : null }><FontAwesomeIcon icon={faStar} /></span></div></div> */}
          ...
          <div className="AddButton">
            <span className="BorderStyle" onClick={submitMod}>
              Add
            </span>
          </div>
        </div>
